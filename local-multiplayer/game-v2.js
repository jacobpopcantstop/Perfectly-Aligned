...
let gameState = {
    ...
    rolledAlignment: null, // The alignment that was rolled (before judge's choice)
    currentPrompts: [],
    selectedPrompt: null,
    submissions: [],
    targetScore: 3,
...
function setupColorPalette() {
    document.querySelectorAll('.color-swatch').forEach(swatch => {
        swatch.addEventListener('click', () => {
            ...
            document.getElementById('colorPicker')?.setAttribute('value', swatch.dataset.color);
        });
    });
}
...
function startGame() {
    ...
    for (let i = 0; i < count; i++) {
        ...
        // Sanitize name
        name = name.replace(/[<>]/g, ''); // Remove potential HTML
    ...
    gameState.timerEnabled = document.getElementById('enableTimer').checked;
...
function drawHandler(e) {
    ...
    const selectedColor = document.querySelector('.color-swatch.selected')?.dataset.color || '#000000';

    if (gameState.currentTool === 'eraser') {
        ctx.globalCompositeOperation = 'destination-out';
        ctx.strokeStyle = 'rgba(0,0,0,1)';
        ctx.lineWidth = parseInt(document.getElementById('brushSize').value) * 2;
    } else {
...
function rollAlignment() {
    ...
    // If Judge's Choice, show selection grid
...
function spendTokenForReroll() {
    ...
    // Deduct token (prioritize in order)
...
function startTimer() {
    ...
    gameState.timerInterval = setInterval(() => {
        ...
        if (gameState.timerSeconds <= 0) {
            clearInterval(gameState.timerInterval);
            alert(`Time's up for ${gameState.players[gameState.currentArtistIndex].name}!`);
            submitDrawing();
        }
    }, 1000);
}
...
function submitDrawing() {
    ...
    if (isCanvasBlank() && !confirm('Your canvas is blank. Submit anyway?')) {
        return;
    }
...
function skipDrawing() {
    if (!confirm(`Skip ${gameState.players[gameState.currentArtistIndex].name}'s turn?`)) return;
    ...
    // Add a blank submission
    gameState.submissions.push({
        playerIndex: gameState.currentArtistIndex,
        playerName: gameState.players[gameState.currentArtistIndex].name,
        drawing: canvas.toDataURL(), // Empty canvas
        skipped: true
    });
...
function startJudgingPhase() {
    ...
    gameState.submissions.forEach((submission, index) => {
        ...
        if (gameState.anonymousMode) {
            card.classList.add('anonymous');
        }

        card.innerHTML = `
            <img src="${submission.drawing}" alt="Drawing">
            <h3>${submission.playerName}${submission.skipped ? ' (Skipped)' : ''}</h3>
        `;
...
function selectWinner(submissionIndex) {
    ...
    if (submission.skipped && !confirm('This player skipped their turn. Select them anyway?')) {
        return;
    }
...
function showResultsPhase() {
    ...
    gameState.submissions.forEach(submission => {
        if (submission.skipped) return; // Don't award tokens to skipped players
...
function openStealModal(playerIndex) {
    ...
    gameState.players.forEach((target, targetIndex) => {
        if (targetIndex === playerIndex) return; // Can't steal from yourself
        if (target.score === 0) return; // Can't steal if they have 0 points
