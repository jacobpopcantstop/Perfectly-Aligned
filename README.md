...
Everything in the game is free: online rooms, offline rooms, all decks, curse cards, token mechanics, and game history access. There is no payment flow, payment tier, or locked feature gate.

### Optional Auth/History Setup

Supabase is only needed if you want authenticated host history persistence.
...
4. Redeploy and verify `GET /api/public-config` returns non-empty `auth.supabaseUrl` and `auth.supabaseAnonKey`.
